package vn.devpro.btck_web.configurer;
...
import vn.devpro.btck_web.service.UserDetailsServiceImpl;
...
public class SecureConfigurer extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		...
		.antMatchers("/frontend/**", "/backend/**", "/FileUploads/**",  "/logout","signin").permitAll()
		...
		.formLogin().loginPage("/signin").loginProcessingUrl("/login_processing_url")
		...
		.failureUrl("/signin?login_error=true")

		.and()

		// Cau hinh phan logout
		.logout().logoutUrl("/logout").logoutSuccessUrl("/signin").invalidateHttpSession(true)
